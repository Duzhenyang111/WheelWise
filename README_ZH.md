...
WheelWise 是一个面向 Codex 的 Superpowers 风格多 skill 包，也是一个 **AI Idea Pre-review Board / AI 产品预评审系统**。它把一个原始产品想法转成中文预评审包：包含预评审状态、证据分类、证据缺口、决策记录、横向比较评分、交付形态建议、复用策略、技术路径、合规与上线前置项、报告可视化网页、独立交互原型，以及可交给 Codex 执行的开发或验证计划。
