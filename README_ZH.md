...
WheelWise V3.0 不再输出散落的 Markdown 单文件。最终产物是一个报告文件夹：
...
V3.0 要求 `index.html` 是经过设计的展示页，使用图片、图表、布局、视觉层级和动效呈现报告内容，不能只是把 Markdown 转成 HTML。凡是有用户可见界面的产品，还需要单独生成类似 `prototype.html` 的交互原型页，用 HTML 模拟网站、网页应用、移动应用、桌面应用、浏览器插件、接口试验台、命令行终端或自动化工具。
...
该示例展示报告文件夹契约、中文 Markdown 报告、设计化 HTML 展示页、产品交互原型和高信息密度视觉资产放置方式。
...
- 用报告文件夹替代单文件输出。
- `report.md` 作为源报告。
- `index.html` 作为设计化中文网页展示层，而不是 Markdown 转换页。
- 为用户可见产品生成独立 HTML 交互原型。
- `assets/` 作为必需资源目录。
- 为完整报告生成高信息密度产品说明图。
- 生成产物的用户可见文字全部中文。
- 通过 `scripts/check_report_contract.py` 强化契约校验。
- 将市场调研、用户发现和商业化作为一级 skill，并加入当前来源调研标准。
