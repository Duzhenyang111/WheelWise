...
其中 `project-state.md` 是内部流程状态，`evidence-board.md` 是内部证据中枢，`report.md` 是源报告，`index.html` 是设计化展示页，`prototype.html` 是产品交互原型，`assets/` 存放图片或图表资产。
...
当用户需要给团队、投资人、潜在客户或自己做验证时，WheelWise 会强化 `visual-brief` 和 `ui-demo`，生成更完整的 `index.html` 展示页和 `prototype.html` 产品原型。
...
| `ui-demo` | 交付形态、用户流程、技术边界 | 展示页规格、原型页规格、模拟数据与状态 | 规划可展示、可点击的产品形态 |
...
它让报告不只是“写得好”，还可以“看得见”。`index.html` 用于展示报告内容，`prototype.html` 用于模拟真实产品界面。网站展示网站流程，网页应用展示工作台，接口产品展示试验台，命令行产品展示终端模拟器，自动化产品展示工作流模拟器。
...
5. `report.md` 是源报告。
6. `index.html` 是设计化展示页，不能只是 Markdown 转 HTML。
...
- 文件夹是否存在。
- 是否包含 `report.md`、`index.html` 和 `assets/`。
